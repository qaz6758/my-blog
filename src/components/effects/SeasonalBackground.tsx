...
const { enabled, activeSeason } = useSeasonalEffect();
...
for (let i = 0; i < particleCount; i++) {
  ...
  if (layer === 0) {
    ...
    particles.push({
      ...
      flipSpeed: 0.009 + Math.random() * 0.01,
    ...
    particles.push({
      ...
      speedX: 0,
      sway: Math.random() * Math.PI * 2,
      swaySpeed: 0.009 + Math.random() * 0.01,
      swayAmp: 1.6,
      angle: Math.random() * Math.PI * 2,
...
const render = () => {
  if (isVisible) {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      const swayOffset =
        Math.sin(p.sway) *
        (p.layer === 0 ? 2.5 : p.layer === 1 ? 5.5 : 9.0);
      ...
      if (p.layer === 0) {
        ...
        ctx.scale(
          p.size,
          p.size * (0.35 + 0.65 * Math.cos(p.flip))
        );
        ctx.globalAlpha = p.opacity;
        ...
        if (isWinter) {
          ...
          // 夏夜微光呼吸起伏
          const breath =
            0.75 + 0.25 * Math.sin(time * 0.04 + p.sway * 2);
